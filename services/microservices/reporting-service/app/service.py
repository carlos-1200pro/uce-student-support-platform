...
class ReportCreate(BaseModel):
    ...
    generated_at: str = Field(min_length=8, max_length=40)


class ReportUpdate(BaseModel):
    title: Optional[str] = Field(default=None, min_length=3, max_length=200)
    generated_at: Optional[str] = Field(default=None, min_length=8, max_length=40)
...
class ReportingService:
    ...
    kafka_topic_events: str = os.getenv("KAFKA_TOPIC_REPORTING_EVENTS", "reporting-events")
    kafka_topic_commands: str = os.getenv("KAFKA_TOPIC_REPORTING_COMMANDS", "reporting-commands")
    rabbit_url: str = os.getenv("RABBIT_URL", "amqp://guest:guest@rabbitmq:5672/")
    rabbit_exchange: str = os.getenv("RABBIT_EXCHANGE", "commands")
    rabbit_queue: str = os.getenv("RABBIT_COMMAND_QUEUE", "reporting-commands")
    rabbit_routing_key: str = os.getenv("RABBIT_ROUTING_KEY", "reporting.command")
    ...
    def list_reports(self) -> list[ReportRead]:
        ...
        with self._db_conn.cursor() as cur:
            cur.execute("SELECT id, title, generated_at FROM reports ORDER BY id ASC")
            rows = cur.fetchall()
        return [ReportRead(id=row[0], title=row[1], generated_at=row[2]) for row in rows]

    def create_report(self, payload: ReportCreate) -> ReportRead:
        self._ensure_ready()
        self._publish_command("reporting.create", payload.model_dump())
        with self._db_conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO reports (title, generated_at)
                VALUES (%s, %s)
                RETURNING id
                """,
                (payload.title, payload.generated_at),
            )
            report_id = cur.fetchone()[0]
        report = ReportRead(id=report_id, title=payload.title, generated_at=payload.generated_at)
        self._emit_event("report.created", report.model_dump())
        return report

    def get_report(self, report_id: int) -> ReportRead:
        self._ensure_ready()
        with self._db_conn.cursor() as cur:
            cur.execute("SELECT id, title, generated_at FROM reports WHERE id = %s", (report_id,))
            row = cur.fetchone()
        if not row:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="report not found")
        return ReportRead(id=row[0], title=row[1], generated_at=row[2])

    def update_report(self, report_id: int, payload: ReportUpdate) -> ReportRead:
        self._ensure_ready()
        self._publish_command("reporting.update", {"report_id": report_id, **payload.model_dump(exclude_unset=True)})
        ...
        with self._db_conn.cursor() as cur:
            cur.execute(
                f"UPDATE reports SET {columns} WHERE id = %s RETURNING id, title, generated_at",
                values,
            )
            row = cur.fetchone()
        if not row:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="report not found")
        report = ReportRead(id=row[0], title=row[1], generated_at=row[2])
        self._emit_event("report.updated", report.model_dump())
        return report

    def delete_report(self, report_id: int) -> bool:
        self._ensure_ready()
        self._publish_command("reporting.delete", {"report_id": report_id})
        with self._db_conn.cursor() as cur:
            cur.execute("DELETE FROM reports WHERE id = %s", (report_id,))
            deleted = cur.rowcount == 1
        if deleted:
            self._emit_event("report.deleted", {"id": report_id})
        return deleted
